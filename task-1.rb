...
def report(file)
  ...
  users.each do |user|
    ...
    time_of_sessions = user_sessions.map { |session| session[:time].to_i }
    user_browsers = user_sessions.map { |session| session[:browser] }
    
    report[:usersStats][user[:full_name]] = {
      # Кол-во сессией по пользователю
      sessionsCount: user_sessions.count,
      # Кол-во времени по пользователям
      totalTime: "#{time_of_sessions.sum} min.",
      # Самая длинная сессия пользователя
      longestSession: "#{time_of_sessions.max} min.",
      # Браузеры пользователя через запятую
      browsers: user_browsers.sort.join(', '),
      # Хоть раз использовал IE?
      usedIE: user_browsers.any? { |browser| browser.upcase =~ /INTERNET EXPLORER/ },
...
data = StackProf.run(mode: :object) do
  report('data_large.txt')
end
...
# StackProf::Report.new(data).print_method(/work/)
