...
def work(lines, file = nil)
  ...
  IO.foreach(file || 'data_large.txt').take(lines).slice_before do |line|
    ...
    users_objects << User.new(attributes: parse_user(user_line.rstrip!), sessions: sessions)
  end

  sessions = users_objects.map(&:sessions).flatten
