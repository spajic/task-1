...
Для того, чтобы понимать, дают ли мои изменения положительный эффект на быстродействие программы я придумал использовать такую метрику: время выполнения (real time)
...
Вот как я построил `feedback_loop`: создал скрипт `run.rb` который позволял быстро запускать бенчмарк либо профилировку
